...
function loadPenagihanData() {
    ...
    window.renderListPenagihan = function() {
        ...
        // Filter data berdasarkan teks pencarian (Bisa cari blok, nama, catatan)
        let filteredData = dataTagihan.filter(item => 
            item.blok.toLowerCase().includes(queryCari) || 
            item.penghuni.toLowerCase().includes(queryCari) ||
            item.catatan.toLowerCase().includes(queryCari)
        );
        ...
        filteredData.forEach((item) => {
            let indexOriginal = dataTagihan.indexOf(item); // Cari index asli untuk fungsi tombol
            
            let statusBadge = ``;
            let borderStyle = ``;
            let uiAksi = ``;
            
            // Tampilan Status Lapangan (Khusus ditaruh di bawah nama)
            let badgeLapangan = item.statusLapangan ? `<span style="background: #f1f5f9; color: #475569; padding: 3px 8px; border-radius: 6px; font-size: 0.7rem; font-weight: bold; border: 1px dashed #cbd5e1; margin-top: 5px; display: inline-block;">📌 ${item.statusLapangan}</span>` : ``;
            let infoCatatan = item.catatan ? `<p style="margin: 8px 0 0 0; font-size: 0.8rem; color: #d97706; font-style: italic; background: #fffbeb; padding: 5px 8px; border-left: 2px solid #f59e0b;">📝 "${item.catatan}"</p>` : ``;

            // LOGIKA UI BERDASARKAN PERAN & SALDO
            ...
            // Gabungkan tombol penagih dan bendahara
            let actionContainer = `
            ...
            listContainer.innerHTML += `
                <div class="card" style="padding: 15px; display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 0; ${borderStyle}">
                    <div style="flex: 1; padding-right: 10px;">
                        <h3 style="margin: 0 0 3px 0; color: #1e293b; font-size: 1rem;">${item.blok}</h3>
